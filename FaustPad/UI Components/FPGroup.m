...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // Initialization code
        uiLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 0, FP_GROUP_LABEL_HEIGHT)];
...
- (void) addComponent:(UIView*)component
{
    if ( [type isEqualToString:@"vgroup"] ) { // vertical layout - add height
        ...
        self.frame = CGRectMake(
                                ...
                                fmax(self.frame.size.height, component.frame.size.height+FP_GROUP_LABEL_HEIGHT)
                                );
    }
